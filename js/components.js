...
var TimePicker = {
  ...
  data: function() {
    ...
    if (this.modelValue) {
      ...
      minute = String((parseInt(parts[1], 10) || 0) % 60).padStart(2, '0');
    }
  ...
  methods: {
    ...
    onManualChange: function() {
      ...
      if (val.indexOf(':') > -1) {
        ...
        if (match) { hour = match[1]; minute = match[2]; }
      } else if (/^\d+$/.test(val)) {
        if (val.length === 4) { hour = val.slice(0, 2); minute = val.slice(2); }
        else if (val.length === 3) { hour = val.slice(0, 1); minute = val.slice(1); }
        else if (val.length <= 2) { hour = '0'; minute = val.padStart(2, '0'); }
        else { hour = val.slice(0, 2); minute = val.slice(2, 4); }
      }
      if (hour !== undefined) {
        this.selectedHour = String(parseInt(hour, 10) % 24).padStart(2, '0');
